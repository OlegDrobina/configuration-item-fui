define("UsrConfItemsFormPageV2_Functions", ["@creatio-devkit/common"], function (sdk) {
	return {
		/*
		...
		*/
		...
		initSysOperations: async function(request) {
			...
			const canEditConfItem = await rightService.getCanExecuteOperation("CanEditConfigurationItem");
		...
		*/
		...
		checkCanUseTeamCityActions: async function(ctx) {
			const operationName = Terrasoft.CurrentUser.userType === Terrasoft.UserType.GENERAL ?
				"SupportCanRunTeamCityAction" :
				"SupportCanRunTeamCityActionFromPortal";
			const rightService = new sdk.RightsService();
			const canRunTeamCityAction = await rightService.getCanExecuteOperation(operationName);
			/*
			...
			*/
			...
			if (canRunTeamCityAction) {
				...
				ctx.TeamCityOperationsCollection = await this.requestTeamCityOperationsList(configurationItemId);
			}
		},
		...
		mapTeamCityActionsVisibility: async function(request) {
			...
			if (Array.isArray(operationsList)) {
				ctx.IsBackupClientDatabaseMenuItemVisible = operationsList.some(item => item?.Name === "Backup client database");
				ctx.IsCreateDemoSiteMenuItemVisible = operationsList.some(item => item?.Name === "Create demo site");
				ctx.IsCreateDemoSiteCopyMenuItemVisible = operationsList.some(item => item?.Name === "Create demo site copy");
				ctx.IsCreateWebsiteCopyMenuItemVisible = operationsList.some(item => item?.Name === "Create website copy");
				ctx.IsDiagnosticsInformationMenuItemVisible = operationsList.some(item => item?.Name === "Diagnostics information");
				ctx.IsFlushRedisMenuItemVisible = operationsList.some(item => item?.Name === "Flush redis");
				ctx.IsGetDatabaseAccessMenuItemVisible = operationsList.some(item => item?.Name === "Get database access");
				ctx.IsGetDumpMemoryMenuItemVisible = operationsList.some(item => item?.Name === "Get dump memory");
				ctx.IsGetTheLogsMenuItemVisible = operationsList.some(item => item?.Name === "Get the logs");
				ctx.IsInstallPackagesMenuItemVisible = operationsList.some(item => item?.Name === "Install packages");
				ctx.IsMoveDemoSiteMenuItemVisible = operationsList.some(item => item?.Name === "Move demo site");
				ctx.IsPasswordResetMenuItemVisible = operationsList.some(item => item?.Name === "Password reset");
				ctx.IsRegistrationInRemoteAccessOnSiteMenuItemVisible = operationsList.some(
					item => item?.Name === "Registration in Remote access OnSite");
				ctx.IsRestartSiteMenuItemVisible = operationsList.some(item => item?.Name === "Restart site");
				ctx.IsWebsiteRelicensingMenuItemVisible = operationsList.some(item => item?.Name === "Website relicensing");
				ctx.IsWebsiteRelicensingNoRedisFlushMenuItemVisible = operationsList.some(item => item?.Name === "Website relicensing (no Redis flush)");
				ctx.IsWebisteVersionUpdateMenuItemVisible = operationsList.some(item => item?.Name === "Website version update");
			}
		},

		openTeamCityActionsPolicy: async function() {
			window.open("https://creatio.atlassian.net/wiki/spaces/TS/pages/2353138060/Performing+actions+on+the+Cloud+platform", "_blank");
		},
		...
		prepareRunTeamCityAction: async function(request) {
			...
			const neededAction = availableTeamCityActions.find(item => item.Name === actionName);
			...
			if (neededAction.MiniPageSchemaName) {
				neededAction.MiniPageSchemaName === "CloudCopyDemoSiteMiniPage" ? 
					await this.processOpenMiniPageRequest(ctx, "UsrCloudCopyDemoSiteMiniPageFUI") :
					await this.processOpenMiniPageRequest(ctx, "UsrCloudMoveDemoSiteMiniPageFUI");
			} else {
				const actionConfig = {
				...
				const result = await this.triggerCorrespondentBusinessProcess(actionConfig);
				if (!result.success) {
					await this.showNotificationMessage(ctx, "Action completed with an error.");
				}
			}
		},
		...
		requestTeamCityOperationsList: async function(confItemId) {
			try {
				...
				const operationsList = response.body.GetOperationListResult?.OperationsList;
		...
		*/
		...
		fillInCurrentStartTimeColumn: async function(request) {
			...
			for (const record of listRecords) {
				...
				if (startTimeUTC) {
					const currentStartTime = await this.processReceivedTime(startTimeUTC, millisecondsToAdd);
		...
		fillInCurrentEndTimeColumn: async function(request) {
			...
			for (const record of listRecords) {
				...
				if (endTimeUTC) {
					const currentEndTime = await this.processReceivedTime(endTimeUTC, millisecondsToAdd);
